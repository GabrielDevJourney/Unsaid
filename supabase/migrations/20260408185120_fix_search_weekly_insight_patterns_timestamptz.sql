-- Fix: created_at return type was `timestamp` (without time zone) but the
-- actual column is `timestamptz`. PostgreSQL error 42804 at runtime.
...
DROP FUNCTION IF EXISTS public.search_weekly_insight_patterns_by_embedding(
    extensions.vector(1536), text, float, int
);
